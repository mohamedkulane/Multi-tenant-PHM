...
CREATE TYPE "LabSensitivity" AS ENUM (
  'NORMAL',
  'SENSITIVE',
  'RESTRICTED'
);
...
CREATE TABLE public.lab_result_amendments (
  ...
  CONSTRAINT lab_result_amendments_test_fk
    FOREIGN KEY (tenant_id, visit_test_id)
    REFERENCES public.lab_visit_tests(tenant_id, id)
    ON DELETE RESTRICT,

  CONSTRAINT lab_result_amendments_actor_fk
    FOREIGN KEY (tenant_id, amended_by_membership_id)
    REFERENCES public.tenant_memberships(tenant_id, id)
    ON DELETE RESTRICT,

  CONSTRAINT lab_result_amendments_reason_check
...
CREATE INDEX lab_result_amendments_tenant_test_amended_idx
  ON public.lab_result_amendments(
    tenant_id,
    visit_test_id,
    amended_at
  );

ALTER TABLE public.lab_result_amendments
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.lab_result_amendments
  FORCE ROW LEVEL SECURITY;

CREATE POLICY lab_result_amendments_tenant_isolation
ON public.lab_result_amendments
USING (
