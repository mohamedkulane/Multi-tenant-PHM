...
export function ClinicalVisitPage({
  ...
  useEffect(() => {
    ...
    setAssessment({
      ...
      symptoms: rows(saved["symptoms"]).map((item) => text(item)).filter(Boolean).join(", "),
  ...
  if (!visit.data) return <EmptyState title="Visit not found" description="Booqashadan lama helin." />;
    ...
    <div className="clinical-visit-page">
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <button className="btn-secondary" onClick={() => window.print()}>
              <Printer size={16} /> Print clinical record
            </button>
          </div>
        }
      />
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Card title="Visit status">
          <div className="p-4"><StatusBadge value={text(visit.data["status"])} /></div>
        </Card>
        <Card title="Phone"><p className="p-4 font-semibold">{text(patient["phone"]) || "Not recorded"}</p></Card>
      ...
      {tab === "assessment" ? (
        ...
        >
          <Card title="Patient interview" description="Record everything manually from the patient interview.">
            <div className="grid gap-4 p-4 lg:grid-cols-2">
              <Field label="Chief complaint">
                <textarea
                  required
                  value={assessment.chiefComplaint}
                  onChange={(event) => setAssessment({ ...assessment, chiefComplaint: event.target.value })}
                />
              </Field>
              <Field label="History of present illness" hint="Onset, duration, progression, severity and associated symptoms.">
                <textarea
              ...
              <Field label="Symptoms" hint="Type symptoms separated by commas or new lines.">
                <textarea
                  ...
                  onChange={(event) => setAssessment({ ...assessment, symptoms: event.target.value })}
                />
              </Field>
              ...
              <Field label="Medicine and other allergies">
                <textarea
                  ...
                  onChange={(event) => setAssessment({ ...assessment, allergies: event.target.value })}
                />
              </Field>
            </div>
          </Card>

          <Card title="Vital signs" description="Enter measured values manually using the displayed units.">
            ...
            <div className="grid gap-4 p-4 lg:grid-cols-2">
              {physicalFields.map(([key, label]) => (
                <Field key={key} label={label}>
                  <textarea
                    value={
                      assessment.physicalExamination[key]
                    }
                    onChange={(event) =>
                      setAssessment({
                        ...assessment,
              ...
              <Field label="Provisional diagnosis" hint="Optional; this does not replace the final diagnosis.">
                <textarea
          ...
          {saveAssessment.error ? <p className="text-sm text-rose-700">{errorMessage(saveAssessment.error)}</p> : null}
          {canDoctor ? (
      ...
      {tab === "laboratory" ? (
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]">
          <Card title="Request laboratory tests" description="Search and select tests from the tenant catalog.">
            <div className="space-y-4 p-4">
              ...
              <div className="grid max-h-[28rem] gap-2 overflow-y-auto sm:grid-cols-2">
                {activeTests.map((test) => {
                  ...
                  return (
                    <label className="flex items-start gap-3 rounded-xl border border-slate-200 p-3" key={id}>
                      <input
                        type="checkbox"
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="text-xs text-slate-500">
                          {text(test["code"])} · {text(test["sampleType"]) || "Sample not specified"} ·{" "}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Field label="Priority">
                  <select value={labPriority} onChange={(event) => setLabPriority(event.target.value)}>
                ...
                <Field label="Clinical notes for laboratory">
                  <textarea value={labNotes} onChange={(event) => setLabNotes(event.target.value)} />
                </Field>
              </div>
              {requestLab.error ? <p className="text-sm text-rose-700">{errorMessage(requestLab.error)}</p> : null}
              {canDoctor ? (
                <button
          ...
          <Card title="Laboratory orders" description={String(labOrders.length) + " order(s) for this visit"}>
                ...
                <div className="space-y-3 p-4" key={text(order["id"])}>
                  ...
                  {rows(order["tests"]).map((test) => (
                    <div className="rounded-lg bg-slate-50 p-3 text-sm" key={text(test["id"])}>
                      ...
                      <p className="mt-1">
                        Result: {text(test["numericValue"]) || text(test["resultValue"]) || text(test["resultStatus"])}
                        {test["unit"] ? " " + text(test["unit"]) : ""}
                      </p>
                      <p className="text-xs text-slate-500">
                        Reference: {text(test["referenceRange"]) || "Not configured"} · Interpretation:{" "}
                        {text(test["interpretation"]) || "Pending"}
                      </p>
                    </div>
                  ))}
                </div>
              )) : <EmptyState title="No lab orders" description="This visit has no laboratory request yet." />}
            </div>
          </Card>
        </div>
      ) : null}
          ...
          <Card title="Recorded diagnoses">
            <div className="space-y-3 p-4">
              {diagnoses.length ? diagnoses.map((diagnosis) => (
                <div className="rounded-xl border border-slate-200 p-3" key={text(diagnosis["id"])}>
              ...
              )) : <EmptyState title="No diagnosis recorded" description="Add the final diagnosis after assessment and any requested lab results." />}
            </div>
          </Card>
          <Card title="Final diagnosis" description="Enter one diagnosis per line. This is always manual.">
            <form
            ...
            >
              ...
              />
              {saveDiagnosis.error ? <p className="text-sm text-rose-700">{errorMessage(saveDiagnosis.error)}</p> : null}
              {canDoctor ? <button className="btn-primary"><Stethoscope size={17} /> Save final diagnosis</button> : null}
            </form>
          </Card>
        </div>
      ) : null}
      ...
      {tab === "prescription" ? (
        <div className="space-y-5">
            ...
            <Card title={"Prescription " + text(prescriptions[0]?.["prescriptionNumber"])}>
              <div className="overflow-x-auto">
                <table className="data-table">
                  <thead><tr><th>Medicine</th><th>Strength</th><th>Dose</th><th>Frequency</th><th>Duration</th><th>Status</th></tr></thead>
                  <tbody>{rows(prescriptions[0]?.["items"]).map((item) => (
                    <tr key={text(item["id"])}>
                      <td>{text(item["medicineName"])}</td><td>{text(item["strength"]) || "—"}</td>
                      <td>{text(item["dosage"])}</td><td>{text(item["frequency"])}</td>
                      <td>{text(item["duration"])}</td><td><StatusBadge value={text(item["status"])} /></td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
          ...
          <Card title="Manual prescription" description="Clinical medicine names are independent from pharmacy inventory products.">
            <form
              className="space-y-4 p-4"
              onSubmit={(event) => {
                event.preventDefault();
            ...
            >
              {prescriptionItems.map((item, index) => (
                <div className="grid gap-3 rounded-xl border border-slate-200 p-4 sm:grid-cols-2 xl:grid-cols-4" key={index}>
                  {[
                        ...
                        onChange={(event) =>
                          setPrescriptionItems(
                            prescriptionItems.map((current, itemIndex) =>
                              itemIndex === index ? { ...current, [String(key)]: event.target.value } : current,
                            ),
                          )
                        }
                      />
              ...
              <button
                ...
                onClick={() =>
                  setPrescriptionItems([
                    ...prescriptionItems,
                    { medicineName: "", strength: "", dosage: "", frequency: "", duration: "", route: "", quantity: "", instructions: "" },
                  ])
                }
              >
              ...
              <Field label="Prescription notes">
                <textarea value={prescriptionNotes} onChange={(event) => setPrescriptionNotes(event.target.value)} />
              </Field>
              {savePrescription.error ? <p className="text-sm text-rose-700">{errorMessage(savePrescription.error)}</p> : null}
              {canDoctor ? <button className="btn-primary"><Save size={17} /> Save prescription</button> : null}
            </form>
          </Card>
        </div>
      ) : null}
